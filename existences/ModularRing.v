...
(*  ModularRing                                     *)
(*                                                  *)
(*  ℤ/nℤ as a CommRingSig instance, for an          *)
(*  arbitrary positive modulus n.                   *)
(*                                                  *)
(*  Design:                                         *)
(*    Carrier := { z : Z | 0 <= z < n }             *)
(*  — a subset type restricting Z to canonical     *)
(*    representatives. Equality on Carrier is      *)
(*    Leibniz (sig_eq_by_value) because the        *)
(*    bound proof is a `bool = true` statement,    *)
(*    which has UIP via `bool_dec`. No extra       *)
(*    proof-irrelevance axiom.                      *)
(*                                                  *)
(*    All operations go through `canonicalize`     *)
(*    (Z.modulo n), guaranteeing the output        *)
(*    stays inside the canonical range.            *)
(*                                                  *)
(*  Concrete instantiation:                         *)
(*    Mod7Ring := ModularRing Mod7Modulus           *)
(*  where n = 7. Matches the modulus used by       *)
(*  results/ModularCoequalizer.v.                  *)
(* ============================================== *)
...
Module ModularRing (M : ModulusSig) <: DecEqCommRingSig.
  ...
  Lemma canonicalize_proj :
    forall z : Z, proj1_sig (canonicalize z) = Z.modulo z M.n.
  Proof. intros. reflexivity. Qed.

  Lemma canonicalize_canonical :
    forall x : Carrier, canonicalize (proj1_sig x) = x.
  Proof.
    intros x. apply sig_eq_by_value. simpl.
