...
(*  IntegerRing                                     *)
(*                                                  *)
(*  The integers ℤ as a CommRingSig instance.       *)
(*  Carrier := Z; operations and axioms delegate    *)
(*  to Coq's ZArith library.                        *)
(*                                                  *)
(*  This is the smallest concrete ring the          *)
(*  framework hosts. Later instances (ModularRing,  *)
(*  PolynomialRing) build on it directly.           *)
(* ============================================== *)
...
Module IntegerRing <: DecEqCommRingSig.
  ...
  Proof. intros. symmetry. apply Z.add_assoc. Qed.

  Theorem add_comm   : forall a b, add a b = add b a.
  Proof. exact Z.add_comm. Qed.

  Theorem add_zero_l : forall a, add zero a = a.
  Proof. exact Z.add_0_l. Qed.

  Theorem add_neg_l  : forall a, add (neg a) a = zero.
  Proof. exact Z.add_opp_diag_l. Qed.

  Theorem mul_assoc  : forall a b c, mul (mul a b) c = mul a (mul b c).
  Proof. intros. symmetry. apply Z.mul_assoc. Qed.

  Theorem mul_one_l  : forall a, mul one a = a.
  Proof. exact Z.mul_1_l. Qed.

  Theorem mul_one_r  : forall a, mul a one = a.
  Proof. exact Z.mul_1_r. Qed.

  Theorem distrib_l  : forall a b c, mul a (add b c) = add (mul a b) (mul a c).
  Proof. exact Z.mul_add_distr_l. Qed.

  Theorem distrib_r  : forall a b c, mul (add a b) c = add (mul a c) (mul b c).
  Proof. exact Z.mul_add_distr_r. Qed.
