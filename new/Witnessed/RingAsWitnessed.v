...
(*  RingAsWitnessed                                 *)
(*                                                  *)
(*  Functor: DecEqCommRingSig -> WitnessedSig.    *)
(*                                                  *)
(*  Entity := Expr × observer_tag (nat).           *)
(*  Expr ::= EConst c | EAdd e1 e2 | EMul e1 e2    *)
(*         | ENeg e                                 *)
(*                                                  *)
(*  The ring operations inhabit Entity as           *)
(*  constructors — the ring's structure lives       *)
(*  inside Entity, not outside.                     *)
(*                                                  *)
(*  interact keeps the expression intact and        *)
(*  advances the observer tag. Ring equations are   *)
(*  expressed through collapse: two syntactically   *)
(*  distinct expressions with equal eval are        *)
(*  linked by collapse.                             *)
(*                                                  *)
(*  No markers. No internal time inside Expr —      *)
(*  the observer tag is a separate component,       *)
(*  understood philosophically as an external       *)
(*  labeling.                                       *)
(* ============================================== *)
...
Module Make (R : DecEqCommRingSig) <: WitnessedSig.
  ...
  Proof.
    exists (EConst R.zero, 0%nat), (EAdd (EConst R.zero) (EConst R.zero), 0%nat).
    intro H. inversion H.
  Qed.
