...
Module FinSetRing (U : UniverseSize) <: DecEqCommRingSig.
  ...
  Lemma and_list_repeat_false_l :
    forall n l, length l = n -> and_list (repeat_bool false n) l = repeat_bool false n.
  Proof.
    induction n as [| n' IH]; intros l H; simpl.
    - reflexivity.
    - destruct l as [| b r]; simpl in H; [discriminate|].
      simpl. f_equal. apply IH. lia.
  Qed.

  Lemma and_list_repeat_false_r :
    forall n l, length l = n -> and_list l (repeat_bool false n) = repeat_bool false n.
  Proof.
    induction n as [| n' IH]; intros l H; simpl.
    - destruct l; simpl in *; [reflexivity | discriminate].
    - destruct l as [| b r]; simpl in H; [discriminate|].
      simpl. rewrite Bool.andb_false_r. f_equal. apply IH. lia.
  Qed.

  Theorem mul_zero_l : forall a, mul zero a = zero.
  Proof.
    intro a. apply sig_eq_by_value. simpl.
    destruct a as [la Ha]. simpl.
    apply and_list_repeat_false_l. exact Ha.
  Qed.
  ...
  Proof.
    intro a. apply sig_eq_by_value. simpl.
    destruct a as [la Ha]. simpl.
    apply and_list_repeat_false_r. exact Ha.
  Qed.
